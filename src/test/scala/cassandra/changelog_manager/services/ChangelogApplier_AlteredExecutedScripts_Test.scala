...
class ChangelogApplier_AlteredExecutedScripts_Test extends Specification with Mockito {

  class MockedChangelogApplier extends ChangelogApplier {
    override def getAlteredExecutedScripts(cqlFiles: List[File], appliedVersions: Set[SchemaVersion]): Set[(String, Try[Boolean])] = super.getAlteredExecutedScripts(cqlFiles, appliedVersions)

    override protected def md5Hash(file: File): HashCode = HashCode.fromString(file.toString)
  }

  "ChangelogApplier.getAlteredExecutedScripts" should {
    "return all altered executed scripts" in {
      //GIVEN
      ...
      val result = mockedChangelogApplier.getAlteredExecutedScripts(
        ...
        Set(notInFilesSV, notAlteredSV, alteredSV1, alteredSV2))

      //THEN
      result shouldEqual Set(("TestCQL3", Success(false)), ("TestCQL4", Success(false)))
      success
    }

    "return nothing when no cql scripts" in {
      //GIVEN
    ...
    "return nothing when no already executed scripts" in {
      //GIVEN
    ...
    "return nothing when all already executed scripts are not altered" in {
      //GIVEN
      val script1 = mockFileWithHashCode("TestCQL1", "287690f6c81cbe07770cec41af87697c")
      val script2 = mockFileWithHashCode("TestCQL2", "7f5a87e1c715f672e43ee51a1de87d46")
    ...
    "make a faillure and continue executing when IO error" in {
      ...
      //THEN
      result shouldEqual Set((ioErrorFile.getName, Failure(exception)), (someChecksumDifferFile.getName, Success(false)))
      success
    }
  }
